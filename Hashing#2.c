//Given a natural language text, store each words in a hash table of size m=SIZE using the mod function.Find the number of key comparisons for a successful search, if the collision resolution technique used is 1) linear probing 2) Quadratic probing
